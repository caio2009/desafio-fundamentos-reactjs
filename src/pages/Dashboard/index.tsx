...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    async function loadTransactions(): Promise<void> {
      ...
      const newTransaction = response.data.transactions.map(
        (el: any): Transaction => {
          return {
            id: el.id,
            title: el.title,
            ...
            formattedValue: formatValue(el.value),
            formattedDate: new Date(el.created_at).toLocaleDateString(),
      ...
      const newBalance = {
        income: formatValue(oldBalance.income),
        outcome: formatValue(oldBalance.outcome),
        total: formatValue(oldBalance.total),
      };
